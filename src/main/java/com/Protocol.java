    ...
    public void setResponse(int function) {
        switch (function) {
            case MSG.PT_RES_MY_REPORT_RECORD_SEND:     // 응답 레포트 목록 전송
                packet = new byte[MSG.HEADER + MSG.FIXED_LEN * 3 + MSG.DOMAIN_LENGTH + MSG.DATE_LENGTH + MSG.FILE_NAME_LENGTH];
                break;
            ...
            case MSG.PT_RES_GUIDELINE_RECORD_SEND:  // 응답 가이드라인 전송
                packet = new byte[MSG.HEADER + (MSG.FIXED_LEN * 3 + MSG.VULNERABILITY_TYPE_LENGTH + MSG.DATE_LENGTH + MSG.FILE_NAME_LENGTH) * 11];
                break;
        }
        size = 0;
